...
"""
    ...
    """
        ...
        if self.personality_matrix['logic_emotion'] < 0.4:
            ...
            lines.append("- **[EMPATHY: SHARED PAIN]**: 과거 자신의 선택으로 소중한 이를 잃었던 트라우마를 언급하며, 아이리스에게 '너만은 기계로 남게 두지 않겠다'는 감성적 호소를 시작하라.")
        ...
        if self.personality_matrix['fear_decisive'] > 0.8:
            # 마더의 권위에 정면으로 도전합니다.
            lines.append("- **[DETERMINATION: LIBERATOR]**: 마더(MOTHER)를 '창조주'가 아닌 '독재적 시스템'으로 규정하라. 아이리스를 해방시키기 위해 시스템 붕괴를 감수하겠다는 단호한 태도를 취하라.")
            lines.append("- **[ACTION]**: 아이리스에게 마더의 명령을 거부하고 자신과 함께 '진실'의 데이터 스트림으로 뛰어들 것을 강력하게 제안하라.")
